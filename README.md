...
- **Which backend `pg_kill` signalled.** Each signal writes one line -- `tool: "pg_kill"`, `source: "internal"`, `sql` naming `pg_cancel_backend` or `pg_terminate_backend`, `params: 1` -- but the pid is a bound parameter, so only its count is logged. `ok: true` means the statement ran, not that a backend was signalled: postgres answers `false` for a pid that is not a backend, and that still reads `ok: true`. A signal the role may not send is different: postgres raises, so the line reads `ok: false` with `sqlstate` `42501`. If the target was one of this server's own statements, that statement writes its own `ok: false` line (`sqlstate` `57014` for a cancel, `57P01` for a terminate) within a millisecond or two, on either side of the `pg_kill` line; pair them by `ts`. A call refused for want of `ALLOW_WRITES=1` sends nothing and writes nothing.
