...
- The scaffolding around agent SQL (`BEGIN READ ONLY`, the row-cap savepoint and cursor, the closing `COMMIT` / `ROLLBACK`), the `pg_type` lookup behind `dataTypeName`, and the `hypopg_create_index` / `hypopg_reset` calls behind `pg_explain`'s `hypothetical_indexes`. None of these gets a line of its own. A `BEGIN`, a savepoint or a `hypopg_create_index` that fails does show, on the statement's own `ok: false` line (see `ok` above), because the statement is never sent; a `pg_type` lookup that fails does not, because the statement already ran and the lookup only decorates its result. `pg_index_advisor` is the exception: everything it sends is logged, including its transaction control and the `EXPLAIN` of each workload statement. The server-version probe some tools run first, `SELECT current_setting('server_version_num') AS v`, is an ordinary `internal` line: once per process on a server that answers, since a successful reading is cached, and on every probing call while the server is unreachable, since a failure is not.
- A call to `pg_health` or `pg_replication_status` that never gets a database connection. These two run several catalog queries on one shared connection and check it out before the first of them is composed, so there is no statement to write a line for, and the trail records statements, not calls; the agent still receives the error. Every other tool records a refused connection as the `ok: false` line of the first statement it was about to send, under its own `tool`: the agent's SQL for `pg_query` and `pg_readonly`; for `pg_explain` the `EXPLAIN` itself, unless a version-floored option (`settings`, `wal`, `generic_plan`, `memory`, `serialize`, `buffers` without `analyze`) or `hypothetical_indexes` was passed, in which case the line is the version probe's or the HypoPG check's and the `EXPLAIN` text is not recorded; the signal statement for `pg_kill`; and the version probe or the first catalog query for the rest. `sqlstate` then holds the Node error code, or is absent for a connect timeout.
