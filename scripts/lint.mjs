 ...
 * build crashes for SOME biome versions. The crash is a property of a
 * particular arm64 build, NOT a permanent defect of the architecture, and it
 * is not caused by npm or by any shell wrapper -- invoking the affected binary
 * directly reproduces it.
 *
 * WHICH builds crash varies by version, so the native path is healthy only by
 * accident of what npm resolved. Measured on this host: arm64 2.5.4 exits 139
 * on a real check, while 2.4.16, 2.5.13 and the 2.5.1 this repo installs all
 * run correctly and check all 34 files.
 *
 ...
 * per-version allowlist that ages badly, or let the gate depend on which build
 * npm last resolved, this script routes Windows ARM64 through the x64 build OF
 * THE SAME VERSION, which runs correctly under Windows' x64 emulation.
 * `YAWLABS_BIOME_NATIVE=1` forces the native binary when you know the
 * installed version's arm64 build is a good one.
 *
 ...
 * there is no .github directory at all, so no runner arbitrates formatting
 * later. Whatever this script reports is the ONLY lint signal that exists
 * before `release.sh` publishes to npm -- step 1 of that script is the only
 * lint gate, as .gitattributes already notes.
 *
 ...
 * than a failed one, with no output to say why. Note that this repo's OTHER
 * unattended release step is not bounded this way -- scripts/run-tests.mjs
 * spawns node with `--test --test-concurrency=1` and no `--test-timeout` --
 * which is all the more reason not to add a second unbounded child here.
 *
 ...
 * roughly 0.2s to 1.1s to check this repo's 34 src/ files through the emulated
 * x64 binary depending on cache warmth -- orders of magnitude under these
 * ceilings, which exist for a hung binary, not a slow one.
 */
 ...
 * bug this replaced: $schema said 2.4.12 while the lockfile installed 2.5.1, so
