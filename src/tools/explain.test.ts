...
describe("pg_explain hypothetical_indexes identifier validation", () => {
  ...
  it("rejects a column name that exceeds 63 bytes (now routed through identSchema)", async () => {
    const overflow = "é".repeat(32);
    const result = (await pgExplain.handler({
      sql: "SELECT 1",
      analyze: false,
    ...
    assert.equal(result.ok, false);
    // The error comes from Zod's refine on identSchema. Surface form may
    // differ from validateHypoIndex's wording, so accept either.
