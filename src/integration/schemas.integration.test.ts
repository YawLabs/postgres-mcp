...
describe("schema tools (integration)", { skip: !integrationEnabled() }, () => {
  before(setupFixtures);
  after(teardownFixtures);

  it("pg_list_schemas returns the fixture schema and excludes pg_catalog", async () => {
  ...
  it("pg_list_tables returns users, posts, and the quoted 'Odd Table'", async () => {
  ...
  it("pg_list_tables with includeViews returns the view too", async () => {
    const res = (await listTables.handler({
      schema: FIXTURE_SCHEMA,
      includeViews: true,
      limit: 500,
  ...
  it("pg_list_tables paginates with limit and offset", async () => {
  ...
  it("pg_describe_table returns columns, PK, FKs, and indexes for posts", async () => {
  ...
  it("pg_describe_table handles quoted identifiers like 'Odd Table'", async () => {
  ...
  it("pg_describe_table returns 'not found' for missing table", async () => {
  ...
  it("pg_list_views returns the user_post_counts view with a definition", async () => {
  ...
  it("pg_list_functions returns the user_count function", async () => {
  ...
  it("pg_list_extensions includes plpgsql (always present)", async () => {
  ...
  it("pg_search_columns finds 'user_id' in posts table", async () => {
  ...
  it("pg_search_columns with wildcard pattern finds 'email'-like columns", async () => {
    const res = (await searchColumns.handler({
      pattern: "%email%",
      schema: FIXTURE_SCHEMA,
      limit: 100,
  ...
  it("pg_search_columns without a schema arg searches across user schemas", async () => {
